...
char fileList[MAX_FILE_NUM][MAX_FILENAME_SIZE];
int versionList[MAX_FILE_NUM];
int matrixElementCountList[MAX_FILE_NUM];
...
int fileCount = 0;
char fileBasename[MAX_FILENAME_SIZE];
int fileIndex = -1;
...
void readMeta(char *path, int *version, int *size) {
    ...
    if (metaFile == NULL) {
        fileIndex = 0;
        strcpy(fileList[fileIndex], base);
        if (fileCount < MAX_FILE_NUM)fileCount++;
        *version = 0;
        *size = 0;
        return;
    }
    ...
    while (count < MAX_FILE_NUM) {
        ...
        int n;
        if (fscanf(metaFile, "%s %d %d", fileList[count], &versionList[count], &matrixElementCountList[count]) == EOF) {
            break;
        }
    ...
    for (i = 0; i < count; ++i) {
        if (strcmp(name, fileList[i]) == 0) {
            ...
            *size = matrixElementCountList[i];
            break;
        }
    }
    if (fileIndex == -1) {
        fileIndex = count < MAX_FILE_NUM ? count : MAX_FILE_NUM - 1;
        strcpy(fileList[fileIndex], base);
        if (fileCount < MAX_FILE_NUM)fileCount++;
    }
    fclose(metaFile);
...
void writeMeta(char *path, int count, int size) {
    ...
    versionList[fileIndex] = count;
    matrixElementCountList[fileIndex] = size;
    int i;
    for (i = 0; i < fileCount; ++i) {
        fprintf(metaFile, "%s %d %d\n", fileList[i], versionList[i], matrixElementCountList[i]);
    }
